...
def aggregate_activations(
    ...
    if use_attention_mask:
        ...
        if method == "mean":
            ...
            seq_lengths = np.sum(attention_mask, axis=1).astype(int) - 1
...
def get_storage_stats(output_dir: str, num_layers: int) -> dict:
    ...
    for layer_idx in range(num_layers):
        ...
        if layer_dir.exists():
            layer_size = sum(
                f.stat().st_size for f in layer_dir.glob("*.npy")
            )
    ...
    return {
        ...
        'avg_per_layer_gb': (total_size / num_layers) / (1024**3) if num_layers > 0 else 0,
