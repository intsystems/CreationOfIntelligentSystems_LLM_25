...
def get_all_classifier_metrics(activations, targets, train_ids, val_ids, test_ids):
    train_activations, val_activations, test_activations = (
        activations[train_ids],
        activations[val_ids],
        activations[test_ids],
    )
    ...
    metrics = compute_classification_metrics(
        test_targets, test_preds, y_val_true=val_targets, y_val_pred_proba=val_preds
    )
    return metrics
...
def main(config: DictConfig):
    ...
    for layer_dir in tqdm(os.listdir(path_to_activations)):
        ...
        activations = np.concatenate(
            [
                np.load(Path(path) / f"batch_{batch_id:04d}.npy")
        ...
        targets = np.array(
            [text_dataset[i][config.target_col] for i in range(len(text_dataset))]
        )
        metrics = get_all_classifier_metrics(
            activations, targets, train_ids, val_ids, test_ids
        )
